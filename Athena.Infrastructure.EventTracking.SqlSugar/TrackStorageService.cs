...
public class TrackStorageService : ITrackStorageService
{
    ...
    public TrackStorageService(ISqlSugarEventTrackingClient sqlSugarClient, ILoggerFactory loggerFactory,
    ...
    public async Task WriteAsync(Track track, CancellationToken cancellationToken = default)
    {
        ...
        if (!isInit)
        {
            ...
            // 读取配置
            var configs = await _sqlSugarClient.Queryable<TrackConfig>()
        ...
        await _sqlSugarClient.Updateable(entity).ExecuteCommandAsync(cancellationToken);
    }
    ...
    {
        return _sqlSugarClient.Deleteable<Track>()
            .Where(p => p.TraceId == traceId)
            .ExecuteCommandAsync(cancellationToken);
    }

    private ISugarQueryable<Track> QueryableNoTracking => _sqlSugarClient.Queryable<Track>();
