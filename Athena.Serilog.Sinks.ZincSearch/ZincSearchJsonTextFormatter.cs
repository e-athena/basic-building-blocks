...
public class ZincSearchJsonTextFormatter : ITextFormatter
{
    /// <summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ZincSearchJsonTextFormatter"/> class.
    /// Uses <see cref="DefaultReservedPropertyRenamingStrategy"/>.
    /// </summary>
    public ZincSearchJsonTextFormatter()
    ...
    /// </param>
    public ZincSearchJsonTextFormatter(IReservedPropertyRenamingStrategy renamingStrategy)
    {
    ...
    public virtual void Format(LogEvent logEvent, TextWriter output)
    {
        ...
        JsonValueFormatter.WriteQuotedJsonString(logEvent.Level.ToString(), output);

        // 添加Timestamp
        output.Write(",\"Timestamp\":");
        JsonValueFormatter.WriteQuotedJsonString(logEvent.Timestamp.ToString("o"), output);

        var traceId = (logEvent.TraceId?.ToString() ?? Activity.Current?.TraceId.ToString()) ?? string.Empty;
        if (traceId != string.Empty)
        {
            // 添加TraceId
            output.Write(",\"TraceId\":");
            JsonValueFormatter.WriteQuotedJsonString(traceId, output);
        }

        var spanId = (logEvent.SpanId?.ToString() ?? Activity.Current?.SpanId.ToString()) ?? string.Empty;
        if (spanId != string.Empty)
        {
            // 添加SpanId
            output.Write(",\"SpanId\":");
            JsonValueFormatter.WriteQuotedJsonString(spanId, output);
        }
