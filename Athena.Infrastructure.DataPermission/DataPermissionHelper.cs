...
public static class DataPermissionHelper
{
    ...
    /// <returns></returns>
    public static IEnumerable<Models.DataPermission> GetList(Assembly assembly, string appId)
    {
    ...
    /// <param name="appId">应用ID</param>
    /// <returns></returns>
    public static IEnumerable<Models.DataPermission> GetList(string assemblyName, string appId)
    {
        var dict = GetAttributeList(assemblyName);
        return dict.Keys
            .Where(p => p.AppId == appId)
            .Select(c => new Models.DataPermission(
                appId,
                c.DisplayName!,
                c.BaseType.Name
            )
        ...
        IList<Models.DataPermission>? permissions)
    {
        ...
        return dict
            ...
            .Select(c =>
            {
                ...
                return new Models.DataPermission(
                    appId,
                    c.DisplayName!,
                    resourceKey
                )
    ...
    {
        permissions ??= new List<Models.DataPermission>();
